...
public interface ProductRepository extends JpaRepository<Product, Long>{
    ...
    @Query(value = "select pd.category_id,pd.avg_rating,pd.name,pd.image,pd.is_activated," +
    ...
    @Query(value = "select pd.category_id,pd.avg_rating,pd.name,pd.image,pd.is_activated,pd.is_deleted,pd.sold_quantity," +
    ...
    @Query(value = "select pd.category_id,pd.name,pd.avg_rating,pd.image,pd.is_activated,pd.is_deleted,pd.sold_quantity," +
            "pd.sale_price,pd.product_id,pd.cost_price,pd.current_quantity,pd.description" +
            " from products as pd inner join categories as c " +
            "ON pd.category_id = c.category_id " +
            "inner join pet as p " +
